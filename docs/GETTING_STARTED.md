...
# V2 Natality: 138.8M births, 1990-2024 (82 columns)
...
# V3 Linked: 74.9M births with infant death data, 2005-2023 (92 columns)
...
|------|------|---------|----------|
| `output/convenience/natality_v2_residents_only.parquet` | 138,582,904 | 80 | **Recommended** — residents only, derived indicators included |
| `output/harmonized/natality_v2_harmonized_derived.parquet` | 138,819,655 | 82 | Full file with foreign residents + restatus columns |
| `output/harmonized/natality_v2_harmonized.parquet` | 138,819,655 | 69 | Harmonized only (no derived indicators) |
...
| File | Rows | Columns | Use case |
|------|------|---------|----------|
| `output/convenience/natality_v3_linked_residents_only.parquet` | 74,785,708 | 90 | **Recommended** — residents only, derived indicators included |
| `output/harmonized/natality_v3_linked_harmonized_derived.parquet` | 74,943,824 | 92 | Full file with foreign residents + restatus columns |
| `output/harmonized/natality_v3_linked_harmonized.parquet` | 74,943,824 | 76 | Harmonized only (no derived indicators) |
...
7. **Check missingness profiles before trend work** — run `python scripts/05_validate/harmonized_missingness.py` to see per-variable per-year null rates and flag structural breaks. See `docs/COMPARABILITY.md` "Known pitfalls" section.

8. **1990-2002 era differences**:
...
9. **Cohort vs period** — the V3 file follows each birth cohort for a full year of mortality experience. This is preferred for multivariate analysis over period files.

10. **Record weight** — for cohort analyses, NCHS recommends **not** applying the weight variable. The unweighted data are used in all validation targets.

11. **Age-at-death comparability note** — starting 2019, NCHS revised how AGED/AGER5/AGER22 are calculated (using birth certificate time-of-birth instead of death certificate). This produces more accurate sub-24-hour age categorization but means the <1 day and 1-day age-at-death categories are not perfectly comparable with 2005-2018. Total neonatal/postneonatal splits are minimally affected.

12. **Bridged race dropped 2020-2023** — NCHS no longer provides bridged race in linked files from 2020+. `maternal_race_bridged4` is null for these years, but `maternal_race_ethnicity_5` is reconstructed from MRACE6 detail codes (see caveat 5 above).
