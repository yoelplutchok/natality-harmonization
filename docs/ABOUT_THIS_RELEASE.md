...
A single stacked Parquet file covering all 35 years of U.S. natality data, with a stable 61-column harmonized schema (plus 10 derived columns). The pipeline resolves five different NCHS record layouts, three certificate revisions, and dozens of field-name/coding changes into one consistent format. Variables span maternal and paternal demographics, prenatal care, medical risk factors, congenital anomalies, infections, delivery details, and birth outcomes.

**Output files:**

| File | Rows | Columns | Description |
|------|------|---------|-------------|
| `natality_v2_harmonized.parquet` | 138,819,655 | 61 | Harmonized birth records |
| `natality_v2_harmonized_derived.parquet` | 138,819,655 | 71 | + derived indicators (LBW, preterm, singleton, etc.) |
| `natality_v2_residents_only.parquet` | 138,582,904 | 69 | **Convenience** — residents only, `restatus`/`is_foreign_resident` dropped |
...
| File | Rows | Columns | Description |
|------|------|---------|-------------|
| `natality_v3_linked_harmonized.parquet` | 74,943,824 | 68 | Birth-side + death-side harmonized |
| `natality_v3_linked_harmonized_derived.parquet` | 74,943,824 | 81 | + derived indicators including neonatal/postneonatal death |
| `natality_v3_linked_residents_only.parquet` | 74,785,708 | 79 | **Convenience** — residents only, `restatus`/`is_foreign_resident` dropped |
...
- Explicit **comparability policy** (`docs/COMPARABILITY.md`) with trend-safe subset guidance
- Common **derived analytic variables** (LBW, preterm, singleton, age categories, neonatal/postneonatal death)
- **Validation artifacts** against official NCHS tabulations (`docs/VALIDATION.md`)
...
- Not every variable is fully trend-safe across all 35 years (see comparability classes in the codebook).
...
- 1990-2002 race bridging is approximate (official NCHS bridged race starts at 2003).
