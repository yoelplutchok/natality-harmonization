...
- [x] **V2.6** — Field-position + parser-completeness pass: 71 harmonized + 13 derived columns (84 total), 38 invariants, 183/183 + 35/35 external targets pass. Corrections: 2004 `ATTEND` reads byte 408 (was 410 — restored 4.1M rows from 100% null to 0.22% null); 2013 `FAGECOMB@182-183` and `RF_CESAR@324` newly parsed (restored ~3.1M rows of father_age and ~3.6M rows of prior_cesarean from 100% null); `prior_cesarean` now populated on revised-cert rows 2005-2013 (30.8% coverage in 2005 climbing to 90.2% in 2013); 2016+ diabetes/hypertension switched from stale URF_* tail block to RF_PDIAB/RF_GDIAB/RF_PHYPE/RF_GHYPE. New columns: `father_age_cat_from_rec11` (categorical father age from FAGEREC11 recode, 2005-2013), `maternal_race_detail_15cat` (15-category mother's race recode from MRACE15, 2014+). Linked-cohort merge uses composite `(CO_SEQNUM, CO_YOD)` per NCHS user-guide spec. Three-valued neonatal/postneonatal booleans. 2021–2023 linked user-guide PDFs committed. V3 linked schema mirrors V2 (78 + 16 = 94 columns).
